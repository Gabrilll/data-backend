...
@Service
@Component
public class HospitalConstructionServiceImpl implements ConstructionService {
    ...
    public boolean construction(BaikePage baikePage) throws Exception {
        ...
        Individual hospitalIndividual = null;
        constructionDAO.addDict(individualName);

        // 查询词典中是否有该实体 有则查询返回 没有则创建返回  true表示这是本名
        hospitalIndividual = constructionDealingService.queryIndividual(individualName, polysemantExplain, url, true, OntologyClassEnum.HOSPITAL).getIndividual();
        constructionDAO.addObjectProperty(hospitalIndividual, "是", hospitalIndividual);
...
//            PictureDownloader.picDownload(picSrc, newPicName, semanticConfig.getPicSavePath() + File.separator + OntologyClassEnum.HARM.getName());
            constructionDAO.addDataProperty(hospitalIndividual, "picSrc", picSrc);
        }

        constructionDAO.addDataProperty(hospitalIndividual, "URL信息来源", url);
        constructionDAO.addDataProperty(hospitalIndividual, "描述", lemmaSummary);
        constructionDAO.addDataProperty(hospitalIndividual, "歧义说明", polysemantExplain);
        ...
        constructionDAO.addDataProperties(hospitalIndividual, parameterNamesFilter, parameterValuesFilter);
        ...
        //医院-成立时间
        String[] time={"时间"};
        constructionDealingService.dealProperty(hospitalIndividual,baikePage,true, Arrays.asList(time),OntologyClassEnum.TIME);

        //医院-事件
        String[] event={"事件"};
        constructionDealingService.dealProperty(hospitalIndividual,baikePage,true,Arrays.asList(event),OntologyClassEnum.EVENT);

        //医院-人物
        String[] person={"人物","医生"};
        constructionDealingService.dealProperty(hospitalIndividual,baikePage,true,Arrays.asList(person),OntologyClassEnum.PERSON);
