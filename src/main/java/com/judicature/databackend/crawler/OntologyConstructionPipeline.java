...
import com.judicature.databackend.util.StringFilter;
...
//        List<String> relationUrls = resultItems.get("relationUrls");
        ...
        parameterNamesFilter=StringFilter.specialCharFilter(parameterNamesFilter);
        List<String> parameterValuesFilter = StringFilter.nonBreakingSpaceFilter(parameterValues);
        parameterValuesFilter=StringFilter.spaceFilter(parameterValuesFilter);
        List<String> parameterNamesFilterAgain = StringFilter.parameterNameFilter(parameterNamesFilter);

        List<String> alias=StringFilter.specialCharFilter(resultItems.get("alias"));
...
//        baikePage.setRelationUrls(relationUrls);
        ...
        baikePage.setCatalogue(categories);
        ...
        // 根据属性特征进行分类 返回的是类型枚举
        OntologyClassEnum ontologyClassEnum = LemmaClassify.classify(categories);
        // 根据分类结果获取对应类型的本体构建对象
        ConstructionService constructionService = LemmaClassify.classify(ontologyClassEnum);
        ...
        if (OntologyConstructionLauncher.pageCount>= semanticConfig.getPageNum()) {
